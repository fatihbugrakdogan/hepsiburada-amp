...
async def get_amp(request: Request, rotated: bool, __amp_source_origin: str):
    ...
    if amp_email_sender:
        ...
        if not __amp_source_origin:
            raise JSONResponse(status_code=400, content={"message":"Çark çevilirken bir hata oluştu. Lütfen tekrar deneyin."},headers=headers)
    ...
    if not (start_date <= current_date <= end_date):
        sleep(5)
        raise JSONResponse(status_code=400, content={"message":"Kampanya 1-7 Haziran 2024 arasında geçerlidir."},headers=headers)
